...
**Every manifest field must be listed in `bin/photos`'s `TOP_SET` or `TOP_PHOTO`.** Those constants are the canonical key order the deterministic writer emits. A field missing from them is *not* silently dropped — the writer round-trips its own output and aborts with `manifest round-trip mismatch — refusing to write`, so an unknown key fails the whole write rather than vanishing from it. That is what lets tooling carry sidecar data beside a manifest edit without fear, but add the key to the constant when you add the field.

One trap the round-trip cannot catch: `"tags": []`. `dump` guards tag normalisation with `if p["tags"]` and the emitter with `unless nil?`, and an empty array is both truthy and non-nil in Ruby, so it round-trips happily and gets written. Clearing tags means deleting the key, which is what `bin/photos tag` does; `fmt` will not undo an empty array once it is in the file.
